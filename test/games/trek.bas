...
1390 IFQ1<1ORQ1>8ORQ2<1ORQ2>8THEN GOTO 1600
...
1540 S3=G(Q1,Q2)-100*K3-10*B3:IFK3=0THEN GOTO 1590
1560 PRINT"COMBAT AREA      CONDITION RED":IFS>200THEN GOTO 1590
...
2080 FORI=1TO9:IFLEFT$(A$,3)<>MID$(A1$,3*I-2,3)THENGOTO2160
...
2310 IFC1>=1ANDC1<9THENGOTO2350
2330 PRINT"   LT. SULU REPORTS, 'INCORRECT COURSE DATA, SIR!'":GOTO1990
2350 X$="8":IFD(1)<0THENX$="0.2"
2360 PRINT"WARP FACTOR (0-";X$;")";:INPUTW1:IFD(1)<0ANDW1>.2THENGOTO2470
2380 IFW1>0ANDW1<=8THENGOTO2490
2390 IFW1=0THENGOTO1990
...
2590 FORI=1TOK3:IFK(I,3)=0THEN GOTO 2700
...
3170 FORI=1TON:S1=S1+X1:S2=S2+X2:IFS1<1ORS1>=9ORS2<1ORS2>=9THEN GOTO 3500
3240 S8=INT(S1)*24+INT(S2)*3-26:IFMID$(Q$,S8,2)="  "THEN GOTO 3360
...
3450 T=T+T8:IFT>T0+T9THEN GOTO 6220
...
3850 IFT>T0+T9THEN GOTO 6220
3860 IF8*Q1+Q2=8*Q4+Q5THEN GOTO 3370
...
4370 INPUT"NUMBER OF UNITS TO FIRE";X:IFX<=0THEN GOTO 1990
4400 IFE-X<0THEN GOTO 4360
4410 E=E-X:IFD(7)<0THENX=X*RND(1)
4450 H1=INT(X/K3):FORI=1TO3:IFK(I,3)<=0THEN GOTO 4670
4480 H=INT((H1/FND(0))*(RND(1)+2)):IFH>.15*K(I,3)THEN GOTO 4530
...
4650 K(I,3)=0:G(Q1,Q2)=G(Q1,Q2)-100:Z(Q1,Q2)=G(Q1,Q2):IFK9<=0THEN GOTO 6370
...
4780 IFC1>=1ANDC1<9THEN GOTO 4850
...
4960 IFX3<1ORX3>8ORY3<1ORY3>8THEN GOTO 5490
5000 PRINT"               ";X3;",";Y3:A$="   ":Z1=X:Z2=Y:GOSUB8830
5050 IFZ3<>0THEN GOTO 4920
5060 A$="+K+":Z1=X:Z2=Y:GOSUB8830:IFZ3=0THEN GOTO 5210
5110 PRINT"*** KLINGON DESTROYED ***":K3=K3-1:K9=K9-1:IFK9<=0THEN GOTO 6370
5150 FORI=1TO3:IFX3=K(I,1)ANDY3=K(I,2)THEN GOTO 5190
5180 NEXTI:I=3
5190 K(I,3)=0:GOTO5430
5210 A$=" * ":Z1=X:Z2=Y:GOSUB8830:IFZ3=0THEN GOTO 5280
5260 PRINT"STAR AT";X3;",";Y3;"ABSORBED TORPEDO ENERGY.":GOSUB6000:GOTO1990
5280 A$=">!<":Z1=X:Z2=Y:GOSUB8830:IFZ3=0THEN GOTO 4760
5330 PRINT"*** STARBASE DESTROYED ***":B3=B3-1:B9=B9-1
5360 IFB9>0ORK9>T-T0-T9THEN GOTO 5400
...
5690 IFD(6)>=0THEN GOTO 5910
5700 PRINT"DAMAGE CONTROL REPORT NOT AVAILABLE":IFD0=0THEN GOTO 1990
5720 D3=0:FORI=1TO8:IFD(I)<0THEND3=D3+.1
5760 NEXTI:IFD3=0THEN GOTO 1990
...
6450 IFINT(I+.5)<1ORINT(I+.5)>8ORINT(J+.5)<1ORINT(J+.5)>8THEN GOTO 6540
6490 A$=">!<":Z1=I:Z2=J:GOSUB8830:IFZ3=1THEN GOTO 6580
...
7320 INPUT"COMPUTER ACTIVE AND AWAITING COMMAND";A:IFA<0THEN GOTO 1990
...
7560 O1$="     ----- ----- ----- ----- ----- ----- ----- -----"
7570 PRINTO1$:FORI=1TO8:PRINTI;:IFH8=0THEN GOTO 7740
...
7970 X$="S":IFB9<2THENX$="":IFB9<1THEN GOTO 8010
...
8220 X=X-A:A=C1-W1:IFX<0THEN GOTO 8350
8250 IFA<0THEN GOTO 8410
8260 IFX>0THEN GOTO 8280
8270 IFA=0THENC1=5:GOTO8290
8280 C1=1
8290 IFABS(A)<=ABS(X)THEN GOTO 8330
...
8410 C1=7
8420 IFABS(A)>=ABS(X)THEN GOTO 8450
8430 PRINT"DIRECTION =";C1+(((ABS(X)-ABS(A))+ABS(X))/ABS(X)):GOTO8460
8450 PRINT"DIRECTION =";C1+(ABS(X)/ABS(A))
8460 PRINT"DISTANCE =";SQR(X^2+A^2):IFH8=1THEN GOTO 1990
8480 NEXTI:GOTO1990
...
8590 R1=FNR(1):R2=FNR(1):A$="   ":Z1=R1:Z2=R2:GOSUB8830:IFZ3=0THEN GOTO 8590
8600 RETURN
